...
abstract class PostgresStore implements Store {
  ...
  async init(pairs: ChainPair[], executionId: string, expiresAt: Date): Promise<void> {
    await this.connect();
    await this.execute(`DELETE FROM ${CHAIN_PAIRS_TABLE} WHERE expires_at < now()`, []);
    await this.execute(`DELETE FROM ${EXECUTIONS_TABLE} WHERE expires_at < now()`, []);

    await this.insertMany(EXECUTIONS_TABLE, EXECUTIONS_COLUMNS, [[executionId, expiresAt]]);
  ...
  private async insertMany(table: string, columns: readonly string[], rows: unknown[][]): Promise<void> {
  ...
  async close(executionId?: string): Promise<void> {
    if (executionId) {
      await this.execute(`DELETE FROM ${CHAIN_PAIRS_TABLE} WHERE execution_id = $1`, [executionId]);
      await this.execute(`DELETE FROM ${EXECUTIONS_TABLE} WHERE id = $1`, [executionId]);
    }
  ...
  async insertSwap(swap: SwapEvent): Promise<void> {
    await this.insertRow(EVENT_TABLE, EVENT_COLUMNS, toEventRow(swap));
  }
  ...
  async query(name: string, params: readonly unknown[]): Promise<number> {
    ...
    const rows = await this.execute(def.sql, params);
    return rows.length;
  }

  async fetchAll<T>(sql: string, params: readonly unknown[]): Promise<T[]> {
    return await this.execute<T>(sql, params);
  }

  protected abstract insertChunk(table: string, columns: readonly string[], rows: unknown[][]): Promise<void>;

  protected abstract insertRow(table: string, columns: readonly string[], row: unknown[]): Promise<void>;

  protected abstract execute<T>(sql: string, params: readonly unknown[]): Promise<T[]>;
}
...
export class PgStore extends PostgresStore {
  ...
  protected async insertChunk(table: string, columns: readonly string[], rows: unknown[][]): Promise<void> {
  ...
  protected async insertRow(table: string, columns: readonly string[], row: unknown[]): Promise<void> {
    const placeholders = columns.map((_, i) => `$${i + 1}`).join(",");
    const sql = `INSERT INTO ${table} (${columns.join(",")}) VALUES (${placeholders})`;
    await this.pool.query(sql, row);
  }

  protected async execute<T>(sql: string, params: readonly unknown[]): Promise<T[]> {
